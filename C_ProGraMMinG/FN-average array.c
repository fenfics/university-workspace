...
void main()
{
    ...
    do
    {
		    ...
		    scanf(" %d", &m);
			if(m < 2 || m > Maxsize){
				printf("error rows and maximum of size is 100\n");
				}
			}while(m < 2 || m > Maxsize);
		    ...
		    scanf(" %d", &n);
			if(n < 1 || n > Maxsize){
				printf("error column and maximum of size is 100\n");
				}
			}while (n < 1 || n > Maxsize);
        ...
        for (i = 0; i < m; i++){
 		...
 		MultipleOfArray(array_A,array_B);
        ShowArray(array_A,array_B,array_C);
        CalAverage(array_C);
